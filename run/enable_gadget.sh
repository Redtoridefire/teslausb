...
then
  mkdir -p "$gadget_root/functions/mass_storage.0/lun.2"
  echo "/backingfiles/boombox_disk.bin" > "$gadget_root/functions/mass_storage.0/lun.2/file"
  echo "TeslaUSB BOOMBOX $(du -h /backingfiles/boombox_disk.bin | awk '{print $1}')" > "$gadget_root/functions/mass_storage.0/lun.2/inquiry_string"
fi
