...
log "Copying BoomBox from archive..."
...
function connectionmonitor {
  ...
  do
    ...
    log "connection dead, killing copy-boombox"
...
log "Copied $NUM_FILES_COPIED boombox file(s), deleted $NUM_FILES_DELETED, skipped $NUM_FILES_SKIPPED previously-copied files, encountered $NUM_FILES_ERROR copy errors and $NUM_FILES_DELETE_ERROR delete errors."

if [ $NUM_FILES_COPIED -gt 0 ]
then
  /root/bin/send-push-message "$TESLAUSB_HOSTNAME:" "Copied $NUM_FILES_COPIED boombox file(s), deleted $NUM_FILES_DELETED, skipped $NUM_FILES_SKIPPED previously-copied files, encountered $NUM_FILES_ERROR copy errors and $NUM_FILES_DELETE_ERROR delete errors."
fi
